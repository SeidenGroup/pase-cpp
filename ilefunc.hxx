 ...
 */
template <typename T> class ILEArgument {
public:
  ILEArgument() = delete;

  static constexpr size_t align() { return alignof(T); }

  static constexpr arg_type_t type() { return sizeof(T); }

  static constexpr result_type_t result_type() { return sizeof(T); }
  ...
  template <typename TInner = T, typename = typename std::enable_if_t<
                                     false == std::is_void<TInner>::value>>
...
#define DefineBaseReturnSubst(T, accessor)                                     \
  template <> T ILEArgument<T>::base_return(ILEarglist_base *base) {           \
    return base->result.accessor;                                              \
  }
...
#define DefineResultType(T, ret)                                               \
  template <> constexpr result_type_t ILEArgument<T>::result_type() {          \
    return ret;                                                                \
  }
...
template <typename T> class ILEArgument<T *> {
public:
  ...
  static constexpr arg_type_t type() { return ARG_MEMPTR; }
 ...
 */
template <typename... TArgs> class ILEArglist {
...
private:
  template <size_t index> void write() {}
...
template <typename TReturn, typename... TArgs> class ILEFunction {
  static_assert(ParameterPackContains<void, TArgs...>::value == false,
                "Argument list must not contain void (use no args instead)");
...
public:
  ILEFunction(const char *path, const char *symbol, int flags = 0) {
    ...
    this->path = path;
    this->symbol = symbol;
    this->signature = {ILEArgument<TArgs>::type()..., ARG_END};
  }
  ...
  void init() {
    ...
    if (_ILESYMX(&this->procedure, this->activation_mark,
   ...
   */
  template <typename TReturnInner = TReturn,
            typename = typename std::enable_if_t<
                (ILEArgument<TReturnInner>::result_type() <= 0)>>
...
private:
  void call(ILEarglist_base *base) {
    ...
    this->init();
    int rc = _ILECALLX(&this->procedure, base, this->signature.data(),
                       ILEArgument<TReturn>::result_type(), this->flags);
